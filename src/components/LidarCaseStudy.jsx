...
const CHAIN_BLOCKS = [
  { x: 0, label: "TUNABLE\nLASER", sub: "FMCW chirp", color: "#5FB3BF", w: 100 },
  { x: 120, label: "SPLITTER", sub: "LO tap", color: "#5FB3BF", w: 90 },
  { x: 230, label: "OPA\nTRANSMITTER", sub: "Beam steering", color: "#5FB3BF", w: 100 },
  { x: 350, label: "TARGET", sub: "Reflection", color: "#e6eaf0", w: 80 },
  { x: 450, label: "COHERENT\nRECEIVER", sub: "90° hybrid + BPD", color: "#5FB3BF", w: 110 },
  { x: 580, label: "DIGITAL\nBACK-END", sub: "FFT: range + velocity", color: "#e6eaf0", w: 90 },
];
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 1; i <= 4; i++) {
        ...
        ctx.strokeStyle = "rgba(95, 179, 191, 0.05)";
        ctx.lineWidth = 1;
        ctx.stroke();
      }
      ...
      for (let i = 0; i <= 6; i++) {
        ...
        ctx.strokeStyle = "rgba(95, 179, 191, 0.04)";
        ctx.stroke();
      }
      ...
      grad.addColorStop(0, "rgba(95, 179, 191, 0.35)");
      ...
      for (let i = points.length - 1; i >= 0; i--) {
        ...
        ctx.fillStyle = `rgba(95, 179, 191, ${(reduced ? 0.35 : 0.6) * life})`;
        ctx.fill();
      }
...
function FmcwChirpSvg({ animate }) {
  return (
    <svg viewBox="0 0 600 280" className="cs-chip-svg" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Frequency versus time plot of FMCW LiDAR: a transmitted chirp ramps up in frequency, the received echo is a time-delayed copy, and their constant difference — the beat frequency — encodes target distance.">
      {/* axes */}
      <line x1="60" y1="230" x2="560" y2="230" stroke="#8b96a5" strokeWidth="0.8" opacity="0.5" />
      <line x1="60" y1="230" x2="60" y2="30" stroke="#8b96a5" strokeWidth="0.8" opacity="0.5" />
      <text x="545" y="248" fontFamily="Space Grotesk, sans-serif" fontSize="9" fill="#8b96a5" opacity="0.6">TIME</text>
      <text x="30" y="40" fontFamily="Space Grotesk, sans-serif" fontSize="9" fill="#8b96a5" opacity="0.6">FREQ</text>

      {/* TX chirp: two up-ramps */}
      <path d="M 60 210 L 280 60 L 280 210 L 500 60" stroke="#5FB3BF" strokeWidth="1.8" fill="none">
      ...
      <line x1="60" y1="210" x2="120" y2="210" stroke="#e6eaf0" strokeWidth="0.8" opacity="0.4" />
      <line x1="60" y1="205" x2="60" y2="215" stroke="#e6eaf0" strokeWidth="0.8" opacity="0.4" />
      <line x1="120" y1="205" x2="120" y2="215" stroke="#e6eaf0" strokeWidth="0.8" opacity="0.4" />
      <text x="72" y="202" fontFamily="monospace" fontSize="8" fill="#e6eaf0" opacity="0.55">τ = 2R/c</text>

      {/* beat frequency between the curves */}
      <line x1="230" y1="94" x2="230" y2="135" stroke="#5FB3BF" strokeWidth="0.8" strokeDasharray="3 2" opacity="0.7" />
      <text x="240" y="120" fontFamily="monospace" fontSize="9" fill="#5FB3BF" opacity="0.8">f_beat ∝ distance</text>

      {/* legend */}
      <line x1="380" y1="240" x2="404" y2="240" stroke="#5FB3BF" strokeWidth="1.8" />
      <text x="410" y="243" fontFamily="Space Grotesk, sans-serif" fontSize="8" fill="#5FB3BF" opacity="0.8">TRANSMITTED</text>
    ...
    <svg viewBox="0 0 600 280" className="cs-chip-svg" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Diagram of an optical phased array: laser light is split into channels, each channel passes a phase shifter, and an array of optical antennas emits a combined wavefront steered off-axis.">
      <defs>
        ...
        <linearGradient id="opaBeam" x1="0" y1="0" x2="1" y2="0">
          <stop stopColor="#5FB3BF" stopOpacity="0.35" />
          <stop offset="1" stopColor="#5FB3BF" stopOpacity="0" />
        </linearGradient>
      </defs>

      <rect x="30" y="20" width="330" height="240" rx="3" fill="url(#opaChip)" stroke="#5FB3BF" strokeWidth="0.8" opacity="0.8" />
      <text x="40" y="42" fontFamily="Space Grotesk, sans-serif" fontSize="9" fill="#5FB3BF" opacity="0.6" letterSpacing="0.15em">SILICON PHOTONIC OPA</text>

      {/* laser input + splitter tree */}
      <line x1="10" y1="140" x2="60" y2="140" stroke="#5FB3BF" strokeWidth="1.5" opacity="0.5" />
      <text x="2" y="132" fontFamily="monospace" fontSize="6" fill="#5FB3BF" opacity="0.5">IN</text>
      {emitters.map((y, i) => (
        <path
          key={`split-${i}`}
      ...
      {emitters.map((y, i) => (
        <g key={`ch-${i}`}>
          <rect x="150" y={y - 6} width="26" height="12" rx="2" fill="none" stroke="#5FB3BF" strokeWidth="0.5" opacity="0.5" />
          <text x="155" y={y + 3} fontFamily="monospace" fontSize="5" fill="#5FB3BF" opacity="0.4">φ{i + 1}</text>
          ...
          {/* grating antenna */}
          {[0, 1, 2, 3].map((k) => (
            <line key={k} x1={304 + k * 8} y1={y - 5} x2={304 + k * 8} y2={y + 5} stroke="#5FB3BF" strokeWidth="1.2" opacity="0.55" />
          ))}
        </g>
      ))}
      <text x="290" y="240" fontFamily="monospace" fontSize="6" fill="#5FB3BF" opacity="0.4">ANTENNA ARRAY</text>
      ...
      {[0, 1, 2].map((k) => (
        <line
          ...
          stroke="#5FB3BF" strokeWidth="0.8" opacity={0.35 - k * 0.08}
        />
      ))}
      <text x="480" y="250" fontFamily="Space Grotesk, sans-serif" fontSize="7" fill="#5FB3BF" opacity="0.4" letterSpacing="0.12em">STEERED BEAM</text>
    </svg>
  );
}
      ...
      <section className="cs-section cs-section--dark">
        ...
        <div className="cs-platform-grid reveal">
          <div className="cs-platform-card">
            <div className="cs-platform-tag" style={{ color: "#5FB3BF", borderColor: "#5FB3BF" }}>FMCW + Coherent Detection</div>
