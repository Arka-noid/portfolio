...
Multi-page consultancy site. **Merilight is Manuel Reza's solo consulting practice** — one engineer, no staff, and the site states that rather than implying a team. **Voice: first person singular on every surface**, including Home/Services/About/Contact; the only "we" on the site is a buyer quoted in their own voice (`data/symptoms.js`). Never use "studio", "agency", "our team" or a firm-voice plural. The /about ÷ /profile split is a subject split, not a voice split: /about is what the practice does and how it is engaged, /profile is the record behind it. **The structure sentences must render** — `firm.structure` on /about and `recordIntro` in `RecordTeaser` were once commented out, which left the site making a firm-shaped impression with the correction switched off. See `docs/positioning.md` §7 before changing any copy's voice. **The thesis: photonics rarely fails on paper — it fails in integration, where optical signals meet custom boards, packaging, thermal limits and control code.** Merilight is the single cross-domain engineer who works on both sides of those boundaries, so a team does not have to hire four specialists to unblock one seam. Lead-generation focused: services come first, career/publications support credibility rather than lead the narrative. A captivating, immersive experience — not a dry resume.
...
| `/about` | `pages/AboutPage.jsx` — the practice: About + RecordTeaser + Publications, as sections on one page |
| `/profile` | `pages/ProfilePage.jsx` — the record behind it: `Principal` + the employment history (`Experience`); deliberately not in the nav |
...
- **Editable prose uses multiline template literals.** In `src/data/**/*.js`, write user-facing paragraphs, descriptions, proof lines, outcomes, publication titles and other long copy with backticks, wrapped across readable source lines. Keep short labels, IDs, URLs, paths, enum-like values and import/export module specifiers as ordinary quoted strings. Rendered page copy uses normal HTML whitespace collapsing. For strings consumed outside rendered HTML, especially SEO metadata, use a trailing `\` on wrapped source lines so editor wrapping does not add runtime whitespace
